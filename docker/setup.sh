...
# Note: generate_nginx_cloudflare_ssl() and generate_compose_cloudflare_ssl_override()
# are provided by docker/_nginx_generators.sh (sourced above).
  ...
  if $USE_CF; then
    ...
    case "$CF_SSL_MODE" in
      2)
        ...
        echo -e "  ${DIM}Paste your ${BOLD}Origin Certificate${RESET}${DIM} below (including BEGIN/END lines).${RESET}"
        echo -e "  ${DIM}Input ends automatically on '-----END CERTIFICATE-----'.${RESET}"
        echo ""
        CERT_CONTENT=""
        ...
        if ! echo "$CERT_CONTENT" | grep -q "BEGIN CERTIFICATE"; then
          error "Invalid certificate — must start with '-----BEGIN CERTIFICATE-----'. Falling back to Flexible mode."
          CF_SSL_MODE=1
        else
          ...
          echo -e "  ${DIM}Paste your ${BOLD}Private Key${RESET}${DIM} below (including BEGIN/END lines).${RESET}"
          echo -e "  ${DIM}Input ends automatically on '-----END ... KEY-----'.${RESET}"
          echo ""
          KEY_CONTENT=""
          while IFS= read -r line; do
            KEY_CONTENT+="${line}"$'\n'
