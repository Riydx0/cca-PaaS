...
generate_nginx_cloudflare_ssl() {
  local domain="$1"
  local cf_ssl_dir="/etc/cloudflare-ssl"
  cat > docker/nginx.conf <<NGINXEOF
# Cloudflare trusted proxy ranges
set_real_ip_from 103.21.244.0/22;
set_real_ip_from 103.22.200.0/22;
...
generate_compose_cloudflare_ssl_override() {
  cat > docker-compose.override.yml <<'OVERRIDEEOF'
# Auto-generated by docker/setup.sh — Cloudflare Full SSL mode
...
if [ -n "$DOMAIN_INPUT" ]; then
  ...
  if $USE_CF; then
    # ── Cloudflare: Flexible or Full? ────────────────────────
    APP_URL="https://${DOMAIN}"
    echo ""
    echo -e "  ${BOLD}Which Cloudflare SSL mode will you use?${RESET}"
    echo -e "  ${DIM}[1] Flexible (recommended) — no certificate needed, fastest setup${RESET}"
    echo -e "  ${DIM}[2] Full — Cloudflare Origin Certificate on port 443 (more secure)${RESET}"
    echo ""
    read -rp "  SSL mode [1/2, default=1]: " CF_SSL_MODE
    echo ""
    ...
    case "$CF_SSL_MODE" in
      2)
        ...
        info "Setting up Cloudflare Full SSL (Origin Certificate)..."
        echo ""
        echo -e "  Generate your certificate at:"
        echo -e "  ${CYAN}Cloudflare Dashboard → ${DOMAIN} → SSL/TLS → Origin Server → Create Certificate${RESET}"
        echo ""
        ...
        echo -e "  ${DIM}Paste your ${BOLD}Origin Certificate${RESET}${DIM} below.${RESET}"
        echo -e "  ${DIM}When done, type ${BOLD}DONE${RESET}${DIM} on a new line and press Enter.${RESET}"
        echo ""
        CERT_CONTENT=""
        ...
        if ! echo "$CERT_CONTENT" | grep -q "BEGIN CERTIFICATE"; then
          error "Invalid certificate — must contain 'BEGIN CERTIFICATE'. Falling back to Flexible mode."
          CF_SSL_MODE=1
        else
          ...
          echo -e "  ${DIM}Paste your ${BOLD}Private Key${RESET}${DIM} below.${RESET}"
          echo -e "  ${DIM}When done, type ${BOLD}DONE${RESET}${DIM} on a new line and press Enter.${RESET}"
          echo ""
          KEY_CONTENT=""
          while IFS= read -r line; do
          ...
          if ! echo "$KEY_CONTENT" | grep -qE "BEGIN (EC |RSA |)PRIVATE KEY"; then
            error "Invalid private key. Falling back to Flexible mode."
            rm -f "$CERT_FILE"
            CF_SSL_MODE=1
          else
            ...
            generate_nginx_cloudflare_ssl "$DOMAIN"
            generate_compose_cloudflare_ssl_override
            COOKIE_SECURE="true"
            success "nginx.conf: Cloudflare Full SSL (HTTPS on port 443)."
            success "docker-compose.override.yml created (port 443 + cert volume)."
            echo ""
            echo -e "  ${DIM}After launch, set Cloudflare SSL/TLS → ${BOLD}Full${RESET}${DIM} in your dashboard.${RESET}"
            echo ""
          fi
        fi
        ;;
    ...
    if [ "$CF_SSL_MODE" != "2" ]; then
      # ── Cloudflare Flexible mode (default) ───────────────────
      ...
      COOKIE_SECURE="false"
      info "Generating nginx config for Cloudflare Flexible..."
      generate_nginx_cloudflare "$DOMAIN"
      rm -f docker-compose.override.yml
      success "nginx.conf: Cloudflare Flexible mode (HTTP origin, HTTPS at edge)."
      echo ""
      echo -e "  ${DIM}Set Cloudflare SSL/TLS → ${BOLD}Flexible${RESET}${DIM} in your dashboard.${RESET}"
