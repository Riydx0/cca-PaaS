...
function ActivityContent({ id }: { id: number }) {
  ...
  const queryStr = useMemo(() => {
    ...
    p.set("limit", "200");
    return p.toString();
  }, [entityType, action, status, from, to]);

  const { data, isLoading, isFetching } = useQuery<{ logs: ActivityLog[] }>({
    queryKey: ["cloudron-instance-activity", id, queryStr],
    queryFn: () => adminFetch<{ logs: ActivityLog[] }>(
  ...
  const hasFilters = entityType !== "all" || action !== "all" || status !== "all" || from !== "" || to !== "";

  function clearFilters() {
    setEntityType("all"); setAction("all"); setStatus("all"); setFrom(""); setTo("");
  }
              ...
              <TableBody>
                {logs.map((a) => (
                  <TableRow key={a.id}>
                    ...
                    <TableCell className="text-sm">{actionLabel(a.action, t)}</TableCell>
